...
#+begin_src emacs-lisp
  ...
  (eval-and-compile
    (setopt use-package-ensure-function #'(lambda () t))
...
#+begin_src emacs-lisp
  (eval-when-compile
    (el-clone :repo "cordx56/rustowl"
            :load-paths `(,(locate-user-emacs-file "el-clone/rustowl/emacs"))))

  (with-delayed-execution
    (add-to-list 'load-path (locate-user-emacs-file "el-clone/rustowl/emacs"))
    (autoload-if-found '(enable-rustowlsp-cursor) "rustowlsp"))
#+end_src
...
#+begin_src emacs-lisp
  (eval-when-compile
    (el-clone :url "https://codeberg.org/ideasman42/emacs-undo-fu.git"
              :repo "emacs-undo-fu"))

  (with-delayed-execution
    (add-to-list 'load-path (locate-user-emacs-file "el-clone/emacs-undo-fu"))
    (autoload-if-found '(undo-fu-only-undo undo-fu-only-redo undo-fu-only-redo-all) "undo-fu"))
#+end_src
...
#+begin_src emacs-lisp
  (eval-when-compile
    (el-clone :repo "MatthewZMD/aidermacs"))

  (with-delayed-execution
    (add-to-list 'load-path (locate-user-emacs-file "el-clone/aidermacs"))
    (autoload-if-found '(aidermacs-transient-menu) "aidermacs")
