...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "C-x t") 'toggle-vterm)
...
#+Begin_src emacs-lisp :tangle no
  (use-package monokai-k
    ...
    :init (doom-themes-neotree-config)
    :hook (after-init-hook . (lambda ()
                               (progn
                                 (require 'doom-monokai-textmate-theme)
...
#+begin_src emacs-lisp :tangle no
  (use-package yascroll
    :ensure t
    :hook (after-init-hook . global-yascroll-bar-mode))
#+end_src
...
*** vterm
#+begin_src emacs-lisp
  (defvar vterm-local-module-dir
    (expand-file-name "var/vterm/" user-emacs-directory)
    "Writable directory for vterm module artifacts.")

  (defun vterm-local--find-executable (name)
    (or (executable-find name)
        (let ((paths
               (list
                (expand-file-name (format "~/.nix-profile/bin/%s" name))
                (format "/etc/profiles/per-user/%s/bin/%s" (user-login-name) name)
                (format "/run/current-system/sw/bin/%s" name)
                (format "/nix/var/nix/profiles/default/bin/%s" name)
                (format "/usr/bin/%s" name)
                (format "/bin/%s" name)))
              found)
  ...
  (defun vterm-local--build-command ()
    (let ((cmake (vterm-local--find-executable "cmake"))
          (nix (vterm-local--find-executable "nix")))
      (cond
       (cmake
        "cmake -G 'Unix Makefiles' -DUSE_SYSTEM_LIBVTERM=On . && make")
       (nix
        (concat
         nix
         " shell nixpkgs#cmake nixpkgs#gnumake nixpkgs#gcc"
         " --command sh -lc "
         (shell-quote-argument "cmake -G 'Unix Makefiles' -DUSE_SYSTEM_LIBVTERM=On . && make")))
       (t
        (error "vterm build needs `cmake` (or `nix` to provision it)")))))

  (defun vterm-local-ensure-module ()
    (let* ((src-lib (locate-library "vterm.el" t))
           (src-dir (and src-lib (file-name-directory src-lib)))
           (stamp-file (expand-file-name ".vterm-source" vterm-local-module-dir))
           (files
            '("CMakeLists.txt"
              "elisp.c"
              "elisp.h"
              ...
              "vterm-module.h")))
      (when src-dir
        (make-directory vterm-local-module-dir t)
        (add-to-list 'load-path vterm-local-module-dir)
        (let ((previous-src
               (when (file-exists-p stamp-file)
                 (with-temp-buffer
                   (insert-file-contents stamp-file)
                   (buffer-string)))))
          (unless (equal previous-src src-dir)
            (dolist (suffix '(".so" ".dylib" ".dll"))
              (let ((module-file (expand-file-name (format "vterm-module%s" suffix) vterm-local-module-dir)))
                (when (file-exists-p module-file)
                  (delete-file module-file))))
            (with-temp-file stamp-file
              (insert src-dir))))
        (dolist (file files)
          (let ((target-file (expand-file-name file vterm-local-module-dir)))
            (when (file-exists-p target-file)
              (delete-file target-file))
            (copy-file (expand-file-name file src-dir)
                       target-file)))
        (unless (let ((load-path (list vterm-local-module-dir)))
                  (locate-library "vterm-module"))
          (let ((default-directory vterm-local-module-dir)
                (buffer (get-buffer-create " *vterm-module-build*")))
            (with-current-buffer buffer
              (erase-buffer))
            (unless (zerop (call-process "sh" nil buffer t "-c" (vterm-local--build-command)))
              (display-buffer buffer)
              (error "Failed to build vterm-module; see %s" (buffer-name buffer))))))))

  (use-package vterm
    :ensure t
    :commands (vterm)
    :init
    (vterm-local-ensure-module))

  (use-package multi-vterm
    :ensure t
    :after vterm
    :commands (multi-vterm multi-vterm-next multi-vterm-prev multi-vterm-dedicated-toggle)
    :config
...
#+begin_src emacs-lisp
  (use-package elcord
    :ensure t
    :hook (after-init-hook . elcord-mode))
#+end_src
