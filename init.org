...
#+begin_src emacs-lisp
  ...
  (defun my/cpp-project-has-compile-database-p (path)
    (let ((dir (if (file-directory-p path)
                   path
                 (file-name-directory path))))
      (or (locate-dominating-file dir "compile_commands.json")
          (locate-dominating-file dir "compile_flags.txt"))))
      ...
      (dolist (buffer (buffer-list))
        (with-current-buffer buffer
          (when (and buffer-file-name
                     ...
                     (not (my/cpp-project-has-compile-database-p buffer-file-name)))
  ...
  (defun my/eglot-cpp-server (_)
    (let* ((g++ (executable-find "g++"))
      ...
      (when g++
        (push (concat "--query-driver=" g++) args)
        (when (and buffer-file-name
                   (not (my/cpp-project-has-compile-database-p buffer-file-name)))
          (when-let ((command (my/eglot-cpp-compile-command buffer-file-name)))
            (setq init
