...
#+begin_src emacs-lisp
  ...
  (defun vterm-local--build-command ()
    (let ((nix (vterm-local--find-executable "nix")))
      (cond
       (nix
        (concat
         nix
         ...
         (shell-quote-argument
          "cmake -G 'Unix Makefiles' -DCMAKE_PREFIX_PATH=/opt/homebrew -DUSE_SYSTEM_LIBVTERM=On . && make")))
       (t
        (error "vterm build needs `nix` to provision build dependencies")))))
