...
(setup meow
  ;; 1. 先确保官方的 meow 核心库已经加载（这样所有内置变量，如 meow-cheatsheet-layout-qwerty 就会立即可用）
  (require 'meow nil t)
  
  ;; 2. 安全加载你自定义的本地 meow 配置（防缺失保护）
  (require 'lib-meow nil t)
...
;; 安全包装：只有当 meow-cjk-mode 函数存在时才尝试加载它
(when (fboundp 'meow-cjk-mode)
  (setup meow-cjk (:hook-into meow-mode)))

(setup sis
  (:idle)
  (:when-loaded
    (require 'lib-sis nil t)
