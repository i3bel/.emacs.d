...
;; 0. 全局系统平台常量定义 (防呆设计)
;; ==========================================
(defconst IS-MAC     (eq system-type 'darwin)  "Whether we are running on macOS.")
(defconst IS-WINDOWS (memq system-type '(ms-dos windows-nt)) "Whether we are running on Windows.")
(defconst IS-LINUX   (eq system-type 'gnu/linux) "Whether we are running on Linux.")
